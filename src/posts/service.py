...
def du_lieu_group_date(group: str,startday: str, endday: str,page:str):
    startday_fix = datetime.strptime(startday, "%d-%m-%Y")
    endday_fix = datetime.strptime(endday, "%d-%m-%Y")
    fd_param = quote_plus(f"{startday_fix.strftime('%d/%m/%Y')} - {endday_fix.strftime('%d/%m/%Y')}")
    url = f"https://vietnamtrademark.net/search?gop=any&g={group}&fd={fd_param}&p={page}"
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
    ...
    return nhan_hieu
def du_lieu_shcn_date(dd_shcn: str,startday: str, endday: str,page:str):
    startday_fix = datetime.strptime(startday, "%d-%m-%Y")
    endday_fix = datetime.strptime(endday, "%d-%m-%Y")
    fd_param = quote_plus(f"{startday_fix.strftime('%d/%m/%Y')} - {endday_fix.strftime('%d/%m/%Y')}")
    url = f"https://vietnamtrademark.net/search?r={dd_shcn}&fd={fd_param}&p={page}"
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
...
def du_lieu_chudon_date(chudon: str,startday: str, endday: str,page:str):
    startday_fix = datetime.strptime(startday, "%d-%m-%Y")
    endday_fix = datetime.strptime(endday, "%d-%m-%Y")
    fd_param = quote_plus(f"{startday_fix.strftime('%d/%m/%Y')} - {endday_fix.strftime('%d/%m/%Y')}")
    url = f"https://vietnamtrademark.net/search?a={chudon}&fd={fd_param}&p={page}"
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
...
def du_lieu_loaidon_date(loaidons: List[LoaiDonEnum],startday: str, endday: str,page:str):
    startday_fix = datetime.strptime(startday, "%d-%m-%Y")
    endday_fix = datetime.strptime(endday, "%d-%m-%Y")
    fd_param = quote_plus(f"{startday_fix.strftime('%d/%m/%Y')} - {endday_fix.strftime('%d/%m/%Y')}")
    if LoaiDonEnum.don_quoc_gia in loaidons:
        url = f"https://vietnamtrademark.net/search?fd={fd_param}&t=0&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
    if LoaiDonEnum.don_quoc_te in loaidons:
        url = f"https://vietnamtrademark.net/search?fd={fd_param}&t=1&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
    try:
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
...
def du_lieu_dd_shcn_loaidon(dd_shcn: str,loaidons: List[LoaiDonEnum],page:str):
    if LoaiDonEnum.don_quoc_gia in loaidons:
        url = f"https://vietnamtrademark.net/search?r={dd_shcn}&t=0&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
    if LoaiDonEnum.don_quoc_te in loaidons:
        url = f"https://vietnamtrademark.net/search?r={dd_shcn}&t=1&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
...
def du_lieu_chudon_loaidon(chudon: str,loaidons: List[LoaiDonEnum],page:str):
    if LoaiDonEnum.don_quoc_gia in loaidons:
        url = f"https://vietnamtrademark.net/search?a={chudon}&t=0&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
    if LoaiDonEnum.don_quoc_te in loaidons:
        url = f"https://vietnamtrademark.net/search?a={chudon}&t=1&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
...
def du_lieu_name_shcn_loaidon(name_sh_ld : str , dd_shcn: str,loaidons: List[LoaiDonEnum],page:str):
    if LoaiDonEnum.don_quoc_gia in loaidons:
        url = f"https://vietnamtrademark.net/search?q={name_sh_ld}&r={dd_shcn}&t=0&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
    if LoaiDonEnum.don_quoc_te in loaidons:
        url = f"https://vietnamtrademark.net/search?q={name_sh_ld}&r={dd_shcn}&t=1&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
...
def du_lieu_name_shcn_date(name_sh_d : str ,dd_shcn: str,startday: str, endday: str,page:str):
    startday_fix = datetime.strptime(startday, "%d-%m-%Y")
    endday_fix = datetime.strptime(endday, "%d-%m-%Y")
    fd_param = quote_plus(f"{startday_fix.strftime('%d/%m/%Y')} - {endday_fix.strftime('%d/%m/%Y')}")
    url = f"https://vietnamtrademark.net/search?q={name_sh_d}&r={dd_shcn}&fd={fd_param}&p={page}"
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
...
def du_lieu_name_chudon_loaidon(name_cd_ld : str ,chudon: str,loaidons: List[LoaiDonEnum],page:str):
    if LoaiDonEnum.don_quoc_gia in loaidons:
        url = f"https://vietnamtrademark.net/search?q={name_cd_ld}&a={chudon}&t=0&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
    if LoaiDonEnum.don_quoc_te in loaidons:
        url = f"https://vietnamtrademark.net/search?q={name_cd_ld}&a={chudon}&t=1&p={page}"
        headers = {"User-Agent": "Mozilla/5.0"}
...
def du_lieu_name_chudon_date(name_cd_d : str ,chudon: str,startday: str, endday: str,page:str):
    startday_fix = datetime.strptime(startday, "%d-%m-%Y")
    endday_fix = datetime.strptime(endday, "%d-%m-%Y")
    fd_param = quote_plus(f"{startday_fix.strftime('%d/%m/%Y')} - {endday_fix.strftime('%d/%m/%Y')}")
    url = f"https://vietnamtrademark.net/search?q={name_cd_d}&a={chudon}&fd={fd_param}&p={page}"
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        resp = requests.get(url, headers=headers)
        resp.raise_for_status()
